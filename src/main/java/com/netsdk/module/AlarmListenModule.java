...
//			System.out.println("Had Subscribe Alarm.");
		...
		if (!LoginModule.netsdk.CLIENT_StartListenEx(LoginModule.m_hLoginHandle)) {
			System.err.printf("CLIENT_StartListenEx Failed!" +  ToolKits.getErrorCodePrint(LoginModule.netsdk.CLIENT_GetLastError()));
			return false;
		} else { 
	...
	public static boolean stopListen() {
		...
		if (!bListening) {
	   	...
	   	if (!LoginModule.netsdk.CLIENT_StopListen(LoginModule.m_hLoginHandle)) { 
			System.err.printf("CLIENT_StopListen Failed!" + ToolKits.getErrorCodePrint(LoginModule.netsdk.CLIENT_GetLastError()));
			return false;
		} else { 
